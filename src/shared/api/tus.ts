...
export async function upload(
  ...
  return new Promise<void | string>((resolve, reject) => {
    const fullBaseURL = baseURL.startsWith("http") ? baseURL : `${origin}${baseURL}`;
    const upload = new tus.Upload(content, {
      endpoint: `${fullBaseURL}${resourcePath}`,
...
export async function useTus(content: ApiContent) {
  return !!tusSettings && isTusSupported() && content instanceof Blob;
}
