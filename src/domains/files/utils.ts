...
export function checkConflict(files: UploadList, dest: ResourceItem[]): boolean {
    if (typeof dest === "undefined" || dest === null) {
        dest = [];
    }

    const folderUpload = files[0].fullPath !== undefined;
    ...
    return new Promise((resolve) => {
        ...
        function readEntry(entry: FileSystemEntry, directory = ""): void {
            if (entry.isFile) {
                ...
                readReaderContent((entry as FileSystemDirectoryEntry).createReader(), `${directory}${entry.name}`);
            }
        }

        function readReaderContent(reader: FileSystemDirectoryReader, directory: string): void {
            reading++;
