  ...
  >
    ...
    <div>
      <p class="name">
        {{ name }}
        <!-- Sharing Indicator -->
        <span v-if="shared" class="material-icons list-share-icon" :title="shareTitle">
          {{ shareIcon }}
        </span>
      </p>
...
import { enableThumbs } from "@/shared/utils/constants";
...
const isSharedByMe = computed(() => {
  if (!props.shared) return false;
  // If ownerId is missing or 0, it's ours. Otherwise check against auth user.
  return !props.ownerId || props.ownerId === 0 || props.ownerId === authStore.user?.id;
});

const shareIcon = computed(() => {
  return isSharedByMe.value ? "share" : "people";
});

const shareTitle = computed(() => {
  return isSharedByMe.value ? "Shared by me" : "Shared with me";
});
